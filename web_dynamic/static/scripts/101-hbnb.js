$(document).ready(function () {
    ...
    $('h2 span').click(function () {
        const span = $(this);
        if (span.text() === 'show') {
            $.getJSON("http://0.0.0.0:5001/api/v1/reviews/", function (data) {
                const reviewList = $('.review-list');
                reviewList.empty();
                for (let review of data) {
                    const reviewItem = $('<div class="review-item">');
                    reviewItem.append($('<h3>').text('From: ' + review.user.first_name + ' ' + review.user.last_name));
                    reviewItem.append($('<p>').text(review.text));
